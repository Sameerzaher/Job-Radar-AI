...
/**
 * Greenhouse apply handler. Opens job URL (boards.greenhouse.io), finds apply form,
 * fills profile fields, uploads resume/cover letter if provided, submits.
 * Returns needs_review if form structure is unsupported or complex steps required.
 */
export async function applyWithGreenhouse(
  _ctx: ApplyAttemptContext
): Promise<ApplyResult> {
  const { job } = _ctx;
  const jobUrl = job.url ?? "";
  if (!jobUrl || !jobUrl.includes("greenhouse")) {
    return { success: false, failureReason: "Invalid or non-Greenhouse URL" };
  }
  ...
  try {
    ...
    try {
      ...
      const applySelector = 'a[href*="apply"], button:has-text("Apply"), a:has-text("Apply for this job")';
      ...
      if (applyVisible) {
        ...
        const formVisible = await page.locator('form[action*="greenhouse"], form[id*="application"]').first().isVisible().catch(() => false);
        if (!formVisible) {
          await browser.close();
      ...
      await fill('input[name*="name"], input[placeholder*="name"], input[id*="name"]', profile.fullName);
      ...
      if (profile.resumeFilePath) {
        const fileInput = await page.locator('input[type="file"][accept*="pdf"], input[type="file"][accept*="document"]').first();
      ...
      if (coverLetter) {
        const coverField = await page.locator('textarea[name*="cover"], textarea[placeholder*="cover"], textarea[id*="cover"]').first();
        if (await coverField.isVisible().catch(() => false)) {
          await coverField.fill(coverLetter).catch(() => {});
        }
      }

      const submitBtn = await page.locator('button[type="submit"], input[type="submit"], button:has-text("Submit"), a:has-text("Submit application")').first();
