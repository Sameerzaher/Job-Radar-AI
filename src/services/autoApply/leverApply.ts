...
/**
 * Lever apply handler. Opens job URL (jobs.lever.co), finds apply form,
 * fills profile fields, uploads resume/cover letter if provided, submits.
 */
export async function applyWithLever(ctx: ApplyAttemptContext): Promise<ApplyResult> {
  const { job } = ctx;
  const jobUrl = job.url ?? "";
  if (!jobUrl || !jobUrl.includes("lever")) {
    return { success: false, failureReason: "Invalid or non-Lever URL" };
  }
